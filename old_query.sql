...
Jobs.media_id AS Jobs__media_id, Jobs.job_category_id AS Jobs__job_category_id, Jobs.job_type_id AS Jobs__job_type_id, Jobs.description AS Jobs__description, Jobs.detail AS Jobs__detail,
...
JobCategories.id AS JobCategories__id, JobCategories.name AS JobCategories__name, JobCategories.sort_order AS JobCategories__sort_order, JobCategories.created_by AS JobCategories__created_by, JobCategories.created AS JobCategories__created, JobCategories.modified AS JobCategories__modified, JobCategories.deleted AS JobCategories__deleted, JobTypes.id AS JobTypes__id,
JobTypes.name AS JobTypes__name, JobTypes.job_category_id AS JobTypes__job_category_id, JobTypes.sort_order AS JobTypes__sort_order, JobTypes.created_by AS JobTypes__created_by, JobTypes.created AS JobTypes__created, JobTypes.modified AS JobTypes__modified, JobTypes.deleted AS JobTypes__deleted
FROM jobs Jobs
LEFT JOIN jobs_personalities JobsPersonalities
ON Jobs.id = (JobsPersonalities.job_id) LEFT JOIN personalities Personalities
...
ON Jobs.id = (JobsBasicAbilities.job_id) LEFT JOIN basic_abilities BasicAbilities
...
LEFT JOIN jobs_career_paths JobsCareerPathsON Jobs.id = (JobsCareerPaths.job_id) LEFT JOIN affiliates CareerPaths
...
ON Jobs.id = (JobsReqQualifications.job_id) LEFT JOIN affiliates ReqQualifications
...
AND (JobCategories.deleted) IS NULL) INNER JOIN job_types JobTypes
...
OR ReqQualifications.name LIKE '​%キャビンアテンダント%​') AND publish_status = 1
